            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                        Administra los iconos de tu sistema
                    </p>
                </div>
            </div>
        </div>
    ...
    <x-m-table :headers="$headers" :rows="$icons" with-pagination>
        ...
        @scope('actions', $icon)
            <div class="flex items-center justify-end gap-1">
                {{-- Editar --}}
                <x-m-button icon="o-pencil" wire:click="edit({{ $icon->id }})" spinner
                    class="btn-sm btn-ghost text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/20"
                    tooltip="Editar" />
                {{-- Eliminar --}}
                <x-m-button icon="o-trash" wire:click="delete({{ $icon->id }})"
                    wire:confirm="¿Estás seguro de que deseas eliminar este icono?" spinner
